...
- **TLS 指纹**：原以为 completion 端点需模拟 Chrome131 指纹（curl_cffi）；**实测（任务 1.1）普通 reqwest（rustls）即可通过，非硬性要求**。其他端点（上传/fork/会话/PoW）普通 httpx 即可。
...
graph TD
    ...
    subgraph dsv-server 原生二进制
        ...
        CFG["~/.deepseek-visionary/config.json<br/>token + smidV2 + 会话状态"]
        SRV --> CFG
    end
...
2. **TLS 指纹：已验证普通 reqwest 即可，无需指纹模拟**
   - **Spike 结论（任务 1.1/1.2，已实测）**：普通 reqwest（rustls）直接 POST completion 端点返回 200 且 SSE 流正常，未被 403。TLS 指纹（curl_cffi chrome131）并非硬性要求，`impersonate` / `tls-client` 方案弃用。
   - 注：Python 版使用 curl_cffi 属历史选择，实测 rustls 指纹同样被接受。
...
5. **自动登录：CDP 浏览器方案（`chromiumoxide`）**
   - 专用登录 profile（`~/.deepseek-visionary/browser/`，目录权限 0700）+ `--remote-debugging-port`。Chrome 136+ 强制要求非默认 profile 才允许远程调试，正好天然满足。
...
6. **凭据与配置：`~/.deepseek-visionary/config.json`（chmod 600）+ 环境变量覆盖**
...
- **TLS 指纹** → 已实测（任务 1.1）非硬性要求，普通 reqwest（rustls）通过，无需指纹模拟；与 Python 版同风险敞口。
...
- **hif-dliq 域名已下线** → 当前环境 DNS 解析失败（Python 版同受其害，流水线卡死）；实测 completion 仅需 `x-hif-leim`，dliq 改为可选：获取失败仅告警跳过。
...
- ~~completion 端点 TLS 指纹是否为硬性要求~~（已定：非硬性，普通 reqwest 即可，见 Decisions 2）。
- ~~`rmcp` 与官方 `mcp` SDK 的 stdio 成熟度对比~~（已定：`rmcp` v2.2，见 Decisions 4）。
- token 有效期与失效自动提示策略（登录工具幂等重跑即可覆盖，无需过度设计）。
- ~~status 工具真实校验 token 的轻量鉴权端点~~（已定：`create_pow_challenge` 探针，任务 1.6 实测通过）。
