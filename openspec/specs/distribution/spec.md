...
## Purpose
定义 `visionary-server` 的发布与分发通道：cargo-dist 多通道安装器（shell / powershell / npm / Homebrew）、MCPB 包保留（MCP Registry 分发）、Registry 元数据随版本更新、裸二进制资产保留（Zed 扩展兼容）与 CI MCP smoke test。
