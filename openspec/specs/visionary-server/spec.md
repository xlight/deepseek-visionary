# visionary-server Specification

## Purpose
定义 `visionary-server` CLI 的跨平台用户目录解析行为：用户目录 SHALL 依次回退 `HOME` → `USERPROFILE`，使 Windows（PowerShell / cmd 无 `HOME`）环境下的 `status` 等子命令与 `skill install` 能正确定位配置目录与技能安装目录，不再回退到当前工作目录。
