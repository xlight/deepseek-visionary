...
graph TD
    subgraph Zed 宿主
        EXT["dsv-zed-ext 扩展壳 (wasm32-wasip2)<br/>下载/定位二进制 + 返回启动命令"]
        EXT -->|spawn 独立进程| SRV
    end
    subgraph dsv-server 原生二进制
        SRV["MCP stdio 服务<br/>vision / status / login / logout 工具"]
        CFG["~/.deepseek-vision/config.json<br/>token + smidV2 + cf_clearance + 会话"]
        SRV --> CFG
    end
...
- **dsv-server**：MCP stdio 服务，实现完整 vision 流水线（PoW → 上传 → fork → HIF 签名 → SSE 流式 completion）与 CDP 自动登录
- **dsv-zed-ext**：Zed 扩展壳，按平台从 GitHub Releases 下载/缓存 dsv-server 并启动

## 安装

1. 从 Zed 扩展市场安装 `DeepSeek Vision`（或在开发模式下安装本地扩展）
...
> `userToken` → 复制 `JSON.parse(value).value`，写入 `~/.deepseek-vision/config.json`：
>
> ```json
> { "user_token": "你的 token" }
> ```
...
会话状态持久化在 `~/.deepseek-vision/session.json`。
...
```bash
# 构建原生服务
cargo build -p dsv-server --release

# 构建扩展壳（wasm32-wasip2）
rustup target add wasm32-wasip2
cargo build -p dsv-zed-ext --release --target wasm32-wasip2

# 测试
cargo test -p dsv-server
```

本地联调：在 Zed 中通过 `Extensions → Install Dev Extension` 选择 `crates/dsv-zed-ext` 目录；
调试时可在扩展设置中配置 `server_path` 指向本地构建的 dsv-server。
...
- **登录**：CDP 控制 Chrome 系浏览器（专用 profile `~/.deepseek-vision/browser/`），读取 `localStorage.userToken` 与 `smidV2` / `cf_clearance` cookie
- **凭据安全**：`~/.deepseek-vision/config.json` 权限 0600，浏览器 profile 0700
